...
const PartsDetails = () => {
    ...
    return (
        <div>
            {
                productDetails.name ? <div className='grid grid-cols-1 mt-11'>
                ...
                <div>
                    <div className='flex justify-center items-center  ml-8 md:ml-0'>
                        <div>
                            <p> <span className='font-medium text-md'> Name:</span> <span className='font-medium'> {name}</span></p>
                            <p> <span className='font-medium text-md'> Available:</span> <span className='font-medium'> {available === 0 ? 'Sold Out' : <>{available ===1 ?  <>{available} Piece</> : <>{available} Pieces</>}</>}</span></p>
                            <p> <span className='font-medium text-md'> Minimum Order:</span> <span className='font-medium'> 1000 Pieces</span></p>
                            <p> <span className='font-medium text-md'> Price:</span> <span className='font-medium'> ${price} Per Unit</span></p>
                            {
                                1000 <= available && <form  onSubmit={e=> e.preventDefault()} className='flex'>
                                                        ...
                                                        <div disabled={orderQuantitys} onClick={()=>{handleQuantity()}} className='mx-3 flex items-center'><PlusCircleIcon className="w-8 text-green-400 bg-blue-100 rounded-full cursor-pointer hover:bg-blue-200 p-1" /></div>
                                                    </form>
                            }
