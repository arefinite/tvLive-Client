...
const Purchase = () => {
    ...
    // 
        ...
        const onSubmit = async data => {
            ...
            await axiosPrivate.put(`https://hidden-harbor-03089.herokuapp.com/inventory/${partsId}`, {restAvailable, userInfo} )
            .then(data=> {
                console.log(data.data);
                if (data.data.modifiedCount === 1){
                    const orderItem = {img,name,totalPrice, email: userInfo, orderQuantity,phone,displayName, address }
                    console.log(order);
                    axiosPrivate.post(`https://hidden-harbor-03089.herokuapp.com/inventory`, {orderItem, userInfo} )
                    .then(data => {
                        if(data.status === 200){
                            console.log("object");
